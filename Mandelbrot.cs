...
namespace WindowsForms_0_
{
    ...
    public class Consts
    {
        public static int WIDTH { get; } = 788;
        public static int HEIGHT { get; } = 788;
   //     public static int MAX_ITER { get; } = 300;
    ...
    //            MessageBox.Show("" + box_for_things.x);
        ...
        void gcol(int counter, int c1, int c2, int i, int j, box_for_things box)
        {
            int red;
            int green;
            int blue;
            ...
            if (counter <= box.max_iter * 1.0 / 10)
                num = 0.1;
            else
                num = 1;
            test = counter / (box.max_iter * num * 0.1);
            num = (num) * box.max_iter;
        ...
        void color_helper(int i, int j, box_for_things box)
        {
            ...
            if (test == box.max_iter)
                array.SetPixel(i, j, Color.FromArgb(0, 0, 0));
            else if (test <= box.max_iter * 1.0 / 10)
                gcol(test, 0x0D1C33, 0x2B6832, i, j, box);
            else
                gcol(test, 0x4F9300, 0xA1D700, i, j, box);
        }
